...
vertex.label.dist <- ifelse(V(g4)$layer == 3, 0,#1.8,   # Outer ring: labels outside
                            ifelse(V(g4)$layer == 2, 0,     # Domains: on node
...
plot(g4_filtered, 
     ...
     #vertex.frame.color = adjustcolor("black", alpha.f = 0.5),
     vertex.frame.color = vertex.frame.color,
     edge.width = E(g4_filtered)$weight / max(E(g4_filtered)$weight, na.rm = TRUE) * 30,#20,
